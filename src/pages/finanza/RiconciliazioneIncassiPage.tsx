...
import {
  Upload, Search, Check, Link2, Plus, Split, EyeOff,
  ArrowDownCircle, Filter, RefreshCw, FileSpreadsheet, AlertCircle
} from "lucide-react";
...
export default function RiconciliazioneIncassiPage() {
  const { user } = useAuth();
  ...
  // Fetch inflow movements
  const { data: movements = [], isLoading } = useQuery({
    queryKey: ["bank-movements-inflow"],
    queryFn: async () => {
  ...
  // Fetch open customer invoices (vendita)
  const { data: openInvoices = [] } = useQuery({
    queryKey: ["open-customer-invoices"],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("invoice_registry")
        .select("*")
        .in("invoice_type", ["vendita", "nota_credito_vendita"])
        .in("financial_status", ["da_incassare", "parzialmente_incassata"])
  ...
  const handleFileUpload = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (movements.length === 0) {
        toast.error("Nessun movimento in entrata trovato nel file");
        return;
      }

      const { error } = await supabase.from("bank_movements").insert(movements as any);
      if (error) throw error;

      toast.success(`${movements.length} movimenti importati con successo`);
      queryClient.invalidateQueries({ queryKey: ["bank-movements-inflow"] });
    } catch (err: any) {
  ...
  const confirmMatch = async (movementId: string) => {
    try {
      ...
      // Update linked invoice financial_status
        ...
        for (const rec of recons) {
          if (rec.invoice_id) {
            const { data: inv } = await supabase
              ...
              .single();
            if (inv) {
              const isVendita = inv.invoice_type?.includes("vendita");
              const newStatus = rec.reconciled_amount >= inv.total_amount
                ? (isVendita ? "incassata" : "pagata")
                : (isVendita ? "parzialmente_incassata" : "parzialmente_pagata");
      ...
      queryClient.invalidateQueries({ queryKey: ["bank-movements-inflow"] });
      queryClient.invalidateQueries({ queryKey: ["open-customer-invoices"] });
    } catch (err: any) {
  ...
  return (
    <div className="p-4 md:p-6 space-y-6 max-w-[1400px] mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-foreground">Riconciliazione Incassi</h1>
          <p className="text-sm text-muted-foreground mt-1">Gestione movimenti bancari in entrata e collegamento fatture clienti</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <Button variant="outline" onClick={() => fileInputRef.current?.click()} disabled={isImporting}>
            ...
            {isImporting ? "Importando..." : "Import Movimenti"}
          </Button>
          <Button onClick={runAutomatch} disabled={isAutoMatching || kpis.unmatched === 0}>
            <RefreshCw className={`h-4 w-4 mr-1 ${isAutoMatching ? "animate-spin" : ""}`} />
            Automatch
          </Button>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16">
          ...
          <p className="text-sm text-muted-foreground">Importa un estratto conto per iniziare la riconciliazione</p>
        </div>
      ) : (
                    ...
                    <TableCell className="text-right">
                      <div className="flex items-center justify-end gap-1">
                        ...
                        {mov.status === "unmatched" && (
                          <Button size="sm" variant="ghost" onClick={() => { setSelectedMovement(mov); setCreateInvoiceDialog(true); }} title="Crea fattura">
      ...
      <Dialog open={linkDialogOpen} onOpenChange={setLinkDialogOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>Collega a Fattura Esistente</DialogTitle>
          </DialogHeader>
          {selectedMovement && (
      ...
      <Dialog open={createInvoiceDialog} onOpenChange={setCreateInvoiceDialog}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>Crea Fattura da Movimento</DialogTitle>
          </DialogHeader>
          {selectedMovement && (
            <CreateInvoiceForm
              ...
              onCancel={() => setCreateInvoiceDialog(false)}
            />
          )}
        </DialogContent>
      </Dialog>
...
function CreateInvoiceForm({ movement, onCreated, onCancel }: {
  ...
  const handleCreate = async () => {
    if (!subjectName || !invoiceNumber) {
      toast.error("Compilare cliente e numero fattura");
      return;
    }
    setSaving(true);
    try {
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        <Label>Cliente *</Label>
        <Input value={subjectName} onChange={(e) => setSubjectName(e.target.value)} placeholder="Nome cliente" />
      </div>
      <div>
        <Label>Numero Fattura *</Label>
        <Input value={invoiceNumber} onChange={(e) => setInvoiceNumber(e.target.value)} placeholder="FV-2026-001" />
      </div>
      <div>
        <Label>Note</Label>
