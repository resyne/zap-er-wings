...
import { Plus, Search, Package, AlertTriangle, TrendingUp, TrendingDown, Building2, ShoppingCart, BoxIcon, MapPin, Euro } from "lucide-react";
...
export default function StockPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      if (materialsError) {
        ...
        toast({ title: "Errore", description: "Errore nel caricamento dei materiali", variant: "destructive" });
        return;
      }
  ...
  const fetchProducts = async () => {
    ...
    try {
      const { data, error } = await supabase
        .from('products')
        .select('id, code, name, description, unit_of_measure, current_stock, minimum_stock, maximum_stock, production_cost, sale_price, warehouse_location')
        .order('name');
  ...
  const handleCreatePurchaseOrder = (material: Material) => {
    if (!material.supplier_id) {
      toast({ title: "Fornitore mancante", description: "Questo materiale non ha un fornitore associato", variant: "destructive" });
      return;
    }
    setSelectedMaterial(material);
    form.reset({ quantity: material.minimum_stock, deliveryTimeframe: "7", priority: "medium", notes: `Riordino per ${material.name}` });
  ...
  const onSubmitPurchaseOrder = async (values: z.infer<typeof purchaseOrderSchema>) => {
    ...
    setEmailPreviewData({
      material: selectedMaterial, supplier, formValues: values, deliveryDays, expectedDeliveryDate,
      estimatedUnitPrice, totalPrice, subtotal: totalPrice, taxAmount: totalPrice * 0.22, totalAmount: totalPrice * 1.22
    });
  ...
  const confirmAndCreateOrder = async () => {
    ...
    try {
      const { error } = await supabase.functions.invoke('create-purchase-order', {
        body: {
          materialId: selectedMaterial.id, quantity: emailPreviewData.formValues.quantity,
          supplierId: selectedMaterial.supplier_id, deliveryTimeframe: emailPreviewData.formValues.deliveryTimeframe,
          priority: emailPreviewData.formValues.priority, notes: emailPreviewData.formValues.notes,
          additionalEmailNotes: additionalEmailNotes,
        },
      });
      if (error) throw error;
      toast({ title: "Ordine creato e inviato", description: "L'ordine di acquisto è stato creato e l'email è stata inviata al fornitore" });
      ...
      toast({ title: "Errore", description: error.message || "Errore nella creazione dell'ordine di acquisto", variant: "destructive" });
    } finally {
  ...
  const getStockStatus = (material: Material) => {
    if (material.current_stock <= material.minimum_stock) return { status: "low", color: "destructive", icon: AlertTriangle };
    if (material.current_stock >= material.maximum_stock) return { status: "high", color: "secondary", icon: TrendingUp };
    return { status: "normal", color: "default", icon: Package };
  };

  const getProductStockStatus = (product: Product) => {
    if (product.current_stock === 0) return { label: "Esaurito", color: "destructive" };
    if (product.minimum_stock && product.current_stock <= product.minimum_stock) return { label: "Sotto scorta", color: "destructive" };
    if (product.maximum_stock && product.current_stock >= product.maximum_stock) return { label: "Scorta alta", color: "secondary" };
  ...
  const productsSummary = {
    ...
    lowStock: products.filter(p => p.minimum_stock && p.current_stock <= p.minimum_stock).length,
    totalValue: products.reduce((sum, p) => sum + (p.current_stock * (p.sale_price || 0)), 0),
  };
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="materiali" className="space-y-6">
            ...
            <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
              ...
              <DialogContent>
                <DialogHeader>
                  ...
                  <DialogDescription>Registra un movimento di carico o scarico</DialogDescription>
                </DialogHeader>
                <div className="space-y-4">
                  ...
                  <div className="flex justify-end space-x-2">
                    ...
                    <Button onClick={() => setIsDialogOpen(false)}>Conferma</Button>
                  </div>
                </div>
              </DialogContent>
            </Dialog>
            ...
            <Dialog open={isPurchaseOrderDialogOpen} onOpenChange={setIsPurchaseOrderDialogOpen}>
              <DialogContent className="sm:max-w-[500px] max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                  ...
                  <DialogDescription>Crea un nuovo ordine di acquisto per {selectedMaterial?.name}</DialogDescription>
                </DialogHeader>
                {selectedMaterial && (
                  <Form {...form}>
                    <form onSubmit={form.handleSubmit(onSubmitPurchaseOrder)} className="space-y-4">
                      <div className="bg-muted p-4 rounded-lg">
                        ...
                        <p className="text-sm text-muted-foreground">Scorta attuale: {selectedMaterial.current_stock} {selectedMaterial.unit}</p>
                        <p className="text-sm text-muted-foreground">Scorta minima: {selectedMaterial.minimum_stock} {selectedMaterial.unit}</p>
                        <p className="text-sm text-muted-foreground">Fornitore: {suppliers.find(s => s.id === selectedMaterial.supplier_id)?.name}</p>
                      </div>
                      ...
                      <FormField control={form.control} name="deliveryTimeframe" render={({ field }) => (
                        <FormItem>
                          <FormLabel>Tempo di consegna richiesto *</FormLabel>
                          <FormControl>
                            <Select onValueChange={field.onChange} value={field.value}>
                              <SelectTrigger><SelectValue placeholder="Seleziona tempo di consegna" /></SelectTrigger>
                              <SelectContent>
                      ...
                      <div className="flex justify-end space-x-2">
                        <Button type="button" variant="outline" onClick={() => setIsPurchaseOrderDialogOpen(false)}>Annulla</Button>
                        <Button type="submit">Anteprima Email</Button>
                      </div>
                    </form>
                  </Form>
                )}
            ...
            <Dialog open={isEmailPreviewOpen} onOpenChange={setIsEmailPreviewOpen}>
              <DialogContent className="sm:max-w-[800px] max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                  ...
                  <DialogDescription>Verifica il contenuto dell'email prima dell'invio e aggiungi eventuali note aggiuntive</DialogDescription>
                </DialogHeader>
                {emailPreviewData && (
                  <div className="space-y-4">
                    <div className="bg-muted p-4 rounded-lg">
                      <h4 className="font-medium mb-2">Destinatario</h4>
                      <p className="text-sm"><strong>{emailPreviewData.supplier?.name}</strong> - {emailPreviewData.supplier?.email || "Email non disponibile"}</p>
                    </div>
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <Input id="additionalNotes" value={additionalEmailNotes} onChange={(e) => setAdditionalEmailNotes(e.target.value)} placeholder="Aggiungi eventuali note aggiuntive che verranno incluse nell'email..." />
                      </div>
                    </div>
                    <div className="flex justify-end space-x-2 pt-4 border-t">
                      <Button type="button" variant="outline" onClick={() => { setIsEmailPreviewOpen(false); setIsPurchaseOrderDialogOpen(true); }}>Modifica Ordine</Button>
                      <Button onClick={confirmAndCreateOrder} disabled={creatingOrder}>{creatingOrder ? "Invio in corso..." : "Conferma e Invia Email"}</Button>
                    </div>
                  </div>
                )}
              </DialogContent>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            ...
            <Card>
              ...
              <CardContent><div className="text-2xl font-bold text-destructive">{stockSummary.lowStock}</div></CardContent>
            </Card>
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              ...
              <CardContent><div className="text-2xl font-bold">€{stockSummary.totalValue.toFixed(2)}</div></CardContent>
            </Card>
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              ...
              <CardContent><div className="text-2xl font-bold text-destructive">{stockSummary.outOfStock}</div></CardContent>
            </Card>
          </div>
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                <div className="flex-1">
                  <div className="relative">
                    ...
                    <Input placeholder="Cerca per nome o codice..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-8" />
                  </div>
                </div>
                ...
                <Select value={selectedType} onValueChange={setSelectedType}>
                  <SelectTrigger className="w-full sm:w-[200px]"><SelectValue placeholder="Tipo Materiale" /></SelectTrigger>
                  <SelectContent>
                ...
                <Select value={selectedSupplier} onValueChange={setSelectedSupplier}>
                  <SelectTrigger className="w-full sm:w-[200px]"><SelectValue placeholder="Fornitore" /></SelectTrigger>
                  <SelectContent>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>{filteredMaterials.length} di {materials.length} materiali</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="rounded-md border">
                <Table>
                  ...
                  <TableBody>
                    {loading ? (
                      <TableRow><TableCell colSpan={11} className="text-center py-8">Caricamento...</TableCell></TableRow>
                    ) : filteredMaterials.length === 0 ? (
                      <TableRow><TableCell colSpan={11} className="text-center py-8 text-muted-foreground">Nessun materiale trovato</TableCell></TableRow>
                    ) : (
                      filteredMaterials.map((material) => {
                        ...
                        return (
                          <TableRow key={material.id}>
                            ...
                            <TableCell><Badge variant="outline">{material.material_type}</Badge></TableCell>
                            <TableCell>{material.category || "-"}</TableCell>
                            <TableCell>
                              {supplier ? (
                                <div className="flex items-center gap-1"><Building2 className="h-3 w-3 text-muted-foreground" /><span className="text-sm">{supplier.name}</span></div>
                              ) : (<span className="text-muted-foreground text-sm">Nessun fornitore</span>)}
                            </TableCell>
                            <TableCell>{material.location || "-"}</TableCell>
                            <TableCell className="text-right"><span className="font-medium">{material.current_stock} {material.unit}</span></TableCell>
                            ...
                            <TableCell>
                              <Badge variant={stockStatus.color as any} className="gap-1">
                                ...
                                {stockStatus.status === "low" ? "Scorta bassa" : stockStatus.status === "high" ? "Scorta alta" : "Normale"}
                              </Badge>
                            </TableCell>
                            <TableCell className="text-right">€{(material.current_stock * material.cost).toFixed(2)}</TableCell>
                            <TableCell>
                              {material.supplier_id && (
                                <Button variant="outline" size="sm" onClick={() => handleCreatePurchaseOrder(material)} className="gap-1">
                                  <ShoppingCart className="h-3 w-3" />Riordina
                                </Button>
                              )}
                            </TableCell>
                          </TableRow>
        ...
        <TabsContent value="prodotti" className="space-y-6">
          <div className="flex justify-end gap-2">
            <Button variant="outline" className="gap-2 border-green-200 text-green-700 hover:bg-green-50" onClick={() => setProductCaricoOpen(true)}>
              <TrendingUp className="h-4 w-4" /> Carico
            </Button>
            <Button variant="outline" className="gap-2 border-red-200 text-red-700 hover:bg-red-50" onClick={() => setProductScaricoOpen(true)}>
              <TrendingDown className="h-4 w-4" /> Scarico
            </Button>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <Card>
              ...
              <CardContent><div className="text-2xl font-bold">{productsSummary.total}</div></CardContent>
            </Card>
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              ...
              <CardContent><div className="text-2xl font-bold text-green-600">{productsSummary.inStock}</div></CardContent>
            </Card>
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">Sotto Scorta</CardTitle>
                <AlertTriangle className="h-4 w-4 text-destructive" />
              </CardHeader>
              <CardContent><div className="text-2xl font-bold text-destructive">{productsSummary.lowStock}</div></CardContent>
            </Card>
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">Valore (Vendita)</CardTitle>
                <Euro className="h-4 w-4 text-muted-foreground" />
              </CardHeader>
              <CardContent><div className="text-2xl font-bold">€{productsSummary.totalValue.toFixed(2)}</div></CardContent>
            </Card>
          </div>
          ...
          <Card>
            <CardContent className="pt-6">
              <div className="relative">
                <Search className="absolute left-2 top-2.5 h-4 w-4 text-muted-foreground" />
                <Input placeholder="Cerca prodotto per nome o codice..." value={productSearchTerm} onChange={(e) => setProductSearchTerm(e.target.value)} className="pl-8" />
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader>
              <CardTitle>Prodotti Finiti in Magazzino</CardTitle>
              <CardDescription>{filteredProducts.length} di {products.length} prodotti</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="rounded-md border">
                <Table>
                  <TableHeader>
                    <TableRow>
                      <TableHead>Codice</TableHead>
                      <TableHead>Nome</TableHead>
                      <TableHead className="text-right">Scorta</TableHead>
                      <TableHead className="text-right">Min/Max</TableHead>
                      <TableHead>Stato</TableHead>
                  ...
                  <TableBody>
                    {productsLoading ? (
                      ...
                      <TableRow><TableCell colSpan={8} className="text-center py-8 text-muted-foreground">Nessun prodotto trovato</TableCell></TableRow>
                    ) : (
                          ...
                          <TableRow key={product.id}>
                            ...
                            <TableCell>
                              <div>
                                ...
                                {product.description && <div className="text-sm text-muted-foreground truncate max-w-[200px]">{product.description}</div>}
                              </div>
                            </TableCell>
                            <TableCell className="text-right font-medium">{product.current_stock} {product.unit_of_measure || "pz"}</TableCell>
                            <TableCell className="text-right text-sm">
                              <div>Min: {product.minimum_stock ?? "-"}</div>
                              <div>Max: {product.maximum_stock ?? "-"}</div>
                            </TableCell>
                            <TableCell>
                              <Badge variant={status.color as any}>{status.label}</Badge>
                            </TableCell>
                            <TableCell className="text-right">{product.production_cost ? `€${product.production_cost.toFixed(2)}` : "-"}</TableCell>
                            <TableCell className="text-right">{product.sale_price ? `€${product.sale_price.toFixed(2)}` : "-"}</TableCell>
                            <TableCell>
                              {product.warehouse_location ? (
                                <div className="flex items-center gap-1"><MapPin className="h-3 w-3 text-muted-foreground" /><span className="text-sm">{product.warehouse_location}</span></div>
                              ) : "-"}
                            </TableCell>
                          </TableRow>
                        );
      ...
      {/* Product Movement Dialogs */}
      <ProductMovementDialog open={productCaricoOpen} onOpenChange={(v) => { setProductCaricoOpen(v); if (!v) fetchProducts(); }} movementType="carico" products={products} />
      <ProductMovementDialog open={productScaricoOpen} onOpenChange={(v) => { setProductScaricoOpen(v); if (!v) fetchProducts(); }} movementType="scarico" products={products} />
    </div>
  );
}
