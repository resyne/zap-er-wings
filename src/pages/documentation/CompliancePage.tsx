...
import { FileText, Download, Upload, Trash2, ShieldCheck } from "lucide-react";
...
export default function CompliancePage() {
  ...
  const onDrop = (acceptedFiles: File[]) => {
    acceptedFiles.forEach((file) => {
      ...
      toast.success(`Dichiarazione caricata - ${languages.find(l => l.code === selectedLanguage)?.name}`);
    });
  };
    ...
    <div className="container mx-auto p-6">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground">
          Gestione certificazioni e dichiarazioni di conformità
        </p>
      </div>
        ...
        <CardContent>
          {filteredDocuments.length === 0 ? (
            <p className="text-center text-muted-foreground py-8">
              Nessuna dichiarazione trovata per la lingua selezionata
            </p>
          ) : (
