...
import { FileText, Download, Upload, Trash2, BookOpen } from "lucide-react";
...
export default function ManualsPage() {
  ...
  const onDrop = (acceptedFiles: File[]) => {
    acceptedFiles.forEach((file) => {
      ...
      toast.success(`Manuale caricato - ${languages.find(l => l.code === selectedLanguage)?.name}`);
    });
  };
    ...
    <div className="container mx-auto p-6">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground">
          Gestione manuali operativi e di manutenzione
        </p>
      </div>
        ...
        <CardContent>
          {filteredDocuments.length === 0 ? (
            <p className="text-center text-muted-foreground py-8">
              Nessun manuale trovato per la lingua selezionata
            </p>
          ) : (
