...
import { ArrowLeft, Search, Loader2, Wrench, Truck, Settings, Calendar, MapPin, User, Package, Clock, ChevronDown, FileText } from "lucide-react";
...
const priorityLabels: Record<string, { label: string; color: string }> = {
  ...
  high: { label: "Alta", color: "text-orange-600 font-semibold" },
  ...
  const { data: workOrders = [], isLoading: loadingWO } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("work_orders")
        .select("id, number, title, status, priority, scheduled_date, location, article, created_at, description, assigned_to, notes, order_id, customers(name)")
        .eq("archived", false)
      ...
      return (data || []).map((wo: any): UnifiedOrder => ({
        ...
        scheduled_date: wo.scheduled_date, location: wo.location, article: wo.article,
        customer_name: wo.customers?.name, created_at: wo.created_at,
        description: wo.description, assigned_to: wo.assigned_to, notes: wo.notes, order_id: wo.order_id,
      }));
    },
  });
  ...
  const { data: serviceOrders = [], isLoading: loadingSWO } = useQuery({
    ...
    queryFn: async () => {
      ...
      return (data || []).map((so: any): UnifiedOrder => ({
        ...
        scheduled_date: so.scheduled_date, location: so.location, article: so.article,
        customer_name: so.customers?.name, created_at: so.created_at,
        description: so.description, assigned_to: so.assigned_to, notes: so.notes, order_id: so.order_id,
      }));
    },
  });
  ...
  const { data: shippingOrders = [], isLoading: loadingSO } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select("id, number, status, shipping_address, shipping_city, article, created_at, notes, customers(name)")
        .eq("archived", false)
        .order("created_at", { ascending: false });
      if (error) throw error;
      return (data || []).map((so: any): UnifiedOrder => ({
        ...
        shipping_address: so.shipping_address, shipping_city: so.shipping_city, notes: so.notes,
      }));
    },
  });
    ...
    return (
      <Collapsible key={cardKey} open={isExpanded} onOpenChange={() => toggleCard(cardKey)}>
        <div className={`bg-white rounded-xl shadow-sm border border-border border-l-4 ${config.borderColor} overflow-hidden`}>
          ...
          {/* Expanded details */}
          <CollapsibleContent>
            <div className={`px-3 pb-3 pt-1 border-t border-border/50 space-y-2 ${config.bgHeader}/30`}>
              ...
              {renderDetailRow(<Calendar className="h-3.5 w-3.5" />, "Data pianificata", order.scheduled_date ? format(new Date(order.scheduled_date), "dd MMMM yyyy", { locale: it }) : null)}
              ...
              {renderDetailRow(<FileText className="h-3.5 w-3.5" />, "Note", order.notes)}
              {order.type === "spedizione" && (
                <>
                  {renderDetailRow(<MapPin className="h-3.5 w-3.5" />, "Indirizzo spedizione", order.shipping_address)}
                  {renderDetailRow(<MapPin className="h-3.5 w-3.5" />, "Città", order.shipping_city)}
                </>
              )}
              {renderDetailRow(<Clock className="h-3.5 w-3.5" />, "Creata il", format(new Date(order.created_at), "dd MMM yyyy, HH:mm", { locale: it }))}
            </div>
          </CollapsibleContent>
        </div>
      </Collapsible>
