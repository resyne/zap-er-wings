...
export function AddMaterialDialog({ open, onOpenChange, suppliers, categories, subcategories }: AddMaterialDialogProps) {
  ...
  useEffect(() => {
    if (open) {
      setForm({
        name: "", code: "", material_type: "componente", unit: "pz",
        minimum_stock: "0", maximum_stock: "100", current_stock: "0",
        supplier_id: "", category_id: "", subcategory_id: "", description: "",
      });
    }
  }, [open]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from("materials").insert({
        ...
        warehouse_category_id: form.category_id || null,
        warehouse_subcategory_id: form.subcategory_id || null,
        description: form.description || null,
        active: true,
      });
      if (error) throw error;
      toast({ title: "Materiale creato con successo" });
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>Aggiungi un nuovo materiale al magazzino</DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              ...
              <Input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="Nome materiale" className="h-9" />
            </div>
            <div className="space-y-1.5">
              <Label className="text-xs font-medium">Codice *</Label>
              <Input value={form.code} onChange={e => setForm(f => ({ ...f, code: e.target.value }))} placeholder="COD-001" className="h-9" />
            </div>
          </div>
          ...
          {suppliers.length > 0 && (
            <div className="space-y-1.5">
              ...
              <Select value={form.supplier_id} onValueChange={v => setForm(f => ({ ...f, supplier_id: v }))}>
                <SelectTrigger className="h-9"><SelectValue placeholder="Seleziona fornitore..." /></SelectTrigger>
                <SelectContent>
                  {suppliers.map(s => (
