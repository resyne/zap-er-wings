...
import { Settings, Plus, Trash2, Pencil, ChevronDown, ChevronRight, Layers, Check, X, Building2 } from "lucide-react";
...
export function WarehouseCategorySettings({ open, onOpenChange, categories, subcategories, suppliers }: Props) {
  ...
  const [newSubSupplier, setNewSubSupplier] = useState("");
  ...
  const addCategory = async () => {
    ...
    const { error } = await supabase.from("warehouse_categories").insert({ name: newCatName.trim(), sort_order: maxOrder + 1 });
  ...
  const updateCategory = async (id: string) => {
    ...
    await supabase.from("warehouse_categories").update({ name: editingCatName.trim() }).eq("id", id);
    setEditingCat(null);
    invalidate();
  };
  ...
  const addSubcategory = async (categoryId: string) => {
    ...
    const { error } = await supabase.from("warehouse_subcategories").insert({
      category_id: categoryId,
      name: newSubName.trim(),
      supplier_id: newSubSupplier || null,
  ...
  const updateSubcategory = async (id: string) => {
    ...
    await supabase.from("warehouse_subcategories").update({ name: editingSubName.trim() }).eq("id", id);
    setEditingSub(null);
    invalidate();
  };

  const deleteSubcategory = async (id: string) => {
    await supabase.from("warehouse_subcategories").delete().eq("id", id);
    invalidate();
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[550px] max-h-[85vh] flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            Gestisci categorie e sottocategorie per organizzare i materiali
          </DialogDescription>
        </DialogHeader>
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                {sub.suppliers?.name && (
                                  <p className="text-[11px] text-muted-foreground flex items-center gap-1">
                                    <Building2 className="h-2.5 w-2.5" /> {sub.suppliers.name}
                                  </p>
                                )}
                              </div>
                      ...
                      {addingSubTo === cat.id ? (
                        <div className="space-y-2 border border-dashed border-border rounded-md p-2">
                          <Input
                            ...
                            autoFocus
                          />
                          <Select value={newSubSupplier} onValueChange={setNewSubSupplier}>
                            <SelectTrigger className="h-8 text-sm">
                              <SelectValue placeholder="Fornitore (opzionale)" />
                            </SelectTrigger>
                            <SelectContent>
                              {suppliers.map(s => (
                          ...
                          <div className="flex gap-1">
                            ...
                            <Button size="sm" variant="outline" className="h-7 text-xs" onClick={() => { setAddingSubTo(null); setNewSubName(""); setNewSubSupplier(""); }}>Annulla</Button>
                          </div>
                        </div>
                      ) : (
                        <Button
