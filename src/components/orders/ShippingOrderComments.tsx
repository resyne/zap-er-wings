...
interface ShippingOrderComment {
  id: string;
  content: string;
  created_at: string;
  user_id: string;
...
export function ShippingOrderComments({ shippingOrderId }: ShippingOrderCommentsProps) {
  const [comments, setComments] = useState<ShippingOrderComment[]>([]);
  ...
  const handleAddComment = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from("shipping_order_comments").insert({
      ...
      if (selectedUsers.length > 0) {
        ...
        for (const userId of selectedUsers) {
          await supabase.from("notifications").insert({
            ...
            message: `${authorName} ti ha menzionato in un commento della commessa di spedizione`,
            type: "mention",
  ...
  const getInitials = (comment: ShippingOrderComment) => {
