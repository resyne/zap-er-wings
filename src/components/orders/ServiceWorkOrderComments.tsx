...
interface ServiceWorkOrderComment {
  id: string;
  content: string;
  created_at: string;
  user_id: string;
...
interface ServiceWorkOrderCommentsProps {
  serviceWorkOrderId: string;
}

export function ServiceWorkOrderComments({ serviceWorkOrderId }: ServiceWorkOrderCommentsProps) {
  const [comments, setComments] = useState<ServiceWorkOrderComment[]>([]);
  ...
  const loadComments = async () => {
    try {
      const { data, error } = await supabase
        ...
        `)
        .eq("service_work_order_id", serviceWorkOrderId)
  ...
  const handleAddComment = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from("service_work_order_comments").insert({
      ...
      if (selectedUsers.length > 0) {
        ...
        for (const userId of selectedUsers) {
          await supabase.from("notifications").insert({
            ...
            message: `${authorName} ti ha menzionato in un commento della commessa di lavoro`,
            type: "mention",
  ...
  const getInitials = (comment: ServiceWorkOrderComment) => {
