...
BEGIN
    -- Only process if the work order has a BOM assigned and status changes to in_progress
    IF NEW.bom_id IS NOT NULL AND NEW.status = 'in_progress' AND (OLD.status IS NULL OR OLD.status != 'in_progress') THEN
        -- Get the main BOM
...
DROP TRIGGER IF EXISTS trigger_reduce_stock_for_production_order ON public.work_orders;
CREATE TRIGGER trigger_reduce_stock_for_production_order
    AFTER INSERT OR UPDATE ON public.work_orders
    FOR EACH ROW
    EXECUTE FUNCTION public.reduce_stock_for_production_order();