-- Controllo di Gestione Database Schema
...
CREATE TABLE public.customer_invoices (
  ...
  aging_days INTEGER GENERATED ALWAYS AS (
    CASE 
      ...
      ELSE EXTRACT(days FROM CURRENT_DATE - due_date)::INTEGER
    END
  ) STORED,
  created_at TIMESTAMPTZ DEFAULT now(),
  updated_at TIMESTAMPTZ DEFAULT now()
...
CREATE TABLE public.service_contracts (
  ...
  renewal_date DATE,
  days_to_renewal INTEGER GENERATED ALWAYS AS (
    EXTRACT(days FROM renewal_date - CURRENT_DATE)::INTEGER
  ) STORED,
  created_at TIMESTAMPTZ DEFAULT now(),
  updated_at TIMESTAMPTZ DEFAULT now()
);
