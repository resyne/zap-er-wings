...
serve(async (req) => {
  ...
  try {
    ...
    const { order_number, supplier_name, total_amount, order_date } = await req.json();

    console.log("Notifying about new purchase order:", { order_number, supplier_name, total_amount });

    const { data: rules, error: rulesError } = await supabase
      .from("zapp_notification_rules")
      .select("*")
      .eq("event_type", "nuovo_ordine_acquisto")
      .eq("is_active", true);
    ...
    const amountFormatted = total_amount ? new Intl.NumberFormat("it-IT", { style: "currency", currency: "EUR" }).format(total_amount) : "N/D";
        ...
        try {
          const waResponse = await fetch(`${supabaseUrl}/functions/v1/whatsapp-send`, {
            ...
            body: JSON.stringify({
              ...
              template_name: "nuovo_ordine_acquisto", template_language: "it",
              template_params: [rule.recipient_name, order_number || "Nuovo OdA", supplier_name || "N/D", amountFormatted, dateFormatted],
            }),
          });
    ...
    for (const rule of rules.filter(r => r.channel === "email")) {
      ...
      try {
        await supabase.from("email_queue").insert({
          ...
          subject: `Nuovo Ordine di Acquisto: ${order_number || ""}`,
          message: `Nuovo OdA: ${order_number}\nFornitore: ${supplier_name}\nImporto: ${amountFormatted}`,
          html_content: `<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:20px;">
            <h2 style="color:#7c3aed;">📦 Nuovo Ordine di Acquisto</h2>
            <p><strong>${order_number || "Nuovo OdA"}</strong></p>
            <ul><li>🏭 Fornitore: ${supplier_name || "N/D"}</li><li>💰 Importo: ${amountFormatted}</li><li>📅 Data: ${dateFormatted}</li></ul>
            <p>Controlla il gestionale per i dettagli.</p></div>`,
          sender_email: "noreply@abbattitorizapper.it", sender_name: "ERP Zapper",
          metadata: { type: "nuovo_ordine_acquisto" },
        });
    ...
    console.error("Error in notify-ordine-acquisto:", error);
